// $Id: strip_spectrumOct.cc,v 2.0 2008/12/05 04:44:04 edwards Exp $
...
struct Wilson_hadron_measurements_t
{
  ...
  Array<PropSource_t>            source_header;
  Array<Real>                    Mass;       // quark mass (bare units)
...
//    read(top, "HybMesP", param.HybMesP);
  ...
  default:
    cerr << "Within Param read - unsupported param version = "
	 << version << endl;
    exit(1);
  }
  ...
  {
    cerr << "Unexpected wvf_param array size" << endl;
    exit(1);
  }
...
//  cout << "Wilson_had = " << path << endl;
  ...
  read(top, "Propagator_info/Masses", had.Mass);
  
  had.source_header.resize(had.Mass.size());
  for(int i(0);i<had.Mass.size();i++){
    ostringstream tag ;
    tag<<"Propagator_info/Propagator["<<i+1<<"]/PropSource" ;
    read(top, tag.str(), had.source_header[i]);
  }

  // Determine what kind of source to use 
  had.Pt_src = ((had.source_header[0].source_type == "POINT_SOURCE")&&
		(had.source_header[1].source_type == "POINT_SOURCE")  )
    ? true : false;
  had.Sl_src = ((had.source_header[0].source_type == "SHELL_SOURCE")||
		(had.source_header[1].source_type == "SHELL_SOURCE") )
    ? true : false;

  // Tag on the 1st prop.
  had.Wl_src = (had.source_header[0].source_type == "WALL_SOURCE")  ? true : false;
...
void print_file(const string& filename, const Array<Real>& mesprop, 
		...
		bool first)
{
  int j_decay = spec.had[0].source_header[0].j_decay;
			 ...
			 int nprop)
{
  ...
  Array<Array<string> > Mass_s(spec.had.size());
  ...
  for(int i = 0; i < spec.had.size(); ++i) 
  {
    snk_wvf_param_s[i].resize(spec.had[i].Mass.size());
    Mass_s[i].resize(spec.had[i].Mass.size());    
    //cout<<"DEBUG(construct_filenames): ";
    //cout<<"Mass.size()="<<spec.had[i].Mass.size()<<endl ;
    // Mass values as an integer
    for (int m=0;m<spec.had[i].Mass.size();m++)
      {
	ostringstream Mass_str;
	Mass_str << int(10000*spec.had[i].Mass[m] + 0.5);
	//cout<<"DEBUG(construct_filenames): ";
	//cout<<i<<" , "<<m<<" -- "<<Mass_str.str()<<endl ;
	Mass_s[i][m] = Mass_str.str();
	//cout<<"DEBUG(construct_filenames): ";
	//cout<<"Mass_s: "<<Mass_s[i][m] <<endl ;
      }
	...
	for(int m(0);m<spec.had[i].Mass.size();m++){
	  if (spec.had[i].source_header[m].sourceSmearParam.wvf_kind == "GAUGE_INV_GAUSSIAN") {
	    wvf_param_str << "G";
	    wvf_param_str << spec.had[i].source_header[m].sourceSmearParam.wvf_param;
	  } 
	  else if(spec.had[i].source_header[m].source_type == "POINT_SOURCE"){
	    wvf_param_str << "P";
	  }
	  else {
	    cerr << "Unknown or unsupported source wvf_kind" << endl;
	    exit(1);
	  }
	...
	if (idx != string::npos)
	  wvf_ps[idx] = 'p';
	idx = wvf_ps.find('.');  // replace '.' with 'p'
	if (idx != string::npos)
	  wvf_ps[idx] = 'p';  
	...
	for(int m(0);m<spec.had[i].Mass.size();m++){
	  ...
	  wvf_param_str << spec.param.wvf_param[m];
	}
    ...
    for(int m(0);m<Mass_s[i].size();m++){
      cout << "  Mass_s[" << i << "]["<<m<<"]= " <<  Mass_s[i][m];
    }
    if (spec.had[i].Sl_src)
  ...
  if (spec.param.MesonP)
  {
    ...
    for(int i = 0; i < spec.had.size(); ++i) 
    {
      int i1=i, i2=i;
      bool diag = true;
      string Mass_12 = ".D" + Mass_s[i][0]+"D"+Mass_s[i][1];
...
//	    cout << spec.had[i].hybmes_SW[m].mom[n].filename << endl;
  ...
  {
    Array<string> baryon_particle(16);
    ...
    Array<string> baryon_smear_state(16);
    ...
    for(int i = 0; i < spec.had.size(); ++i) 
    {
      int i1=i, i2=i, i3=i;
      bool diag = true;
      string Mass_123 = ".D" + Mass_s[i][0] +"D" + Mass_s[i][1];
	...
	for(int m=0; m < files.had[i].baryon_PP.size(); ++m)
	{
	  ...
	  files.had[i].baryon_PP[m].mom.resize(spec.had[i].baryon_PP[m].mom.size());
	
	  for(int n = 0; n < spec.had[i].baryon_PP[m].mom.size(); ++n)
	  {
	    files.had[i].baryon_PP[m].mom[n].filename = baryon_particle[m] 
	      + mom_name(spec.had[i].baryon_PP[m].mom[n].sink_mom) 
	...
	if ( spec.had[i].Pt_src && spec.param.Sl_snk ) 
	{
	  ...
	  for(int m=0; m < files.had[i].baryon_PS.size(); ++m)
	  {
	    ...
	    files.had[i].baryon_PS[m].mom.resize(spec.had[i].baryon_PS[m].mom.size());

	    for(int n = 0; n < spec.had[i].baryon_PS[m].mom.size(); ++n)
	    {
	      files.had[i].baryon_PS[m].mom[n].filename = baryon_particle[m] 
		+ mom_name(spec.had[i].baryon_PS[m].mom[n].sink_mom) 
	...
	if ( spec.had[i].Pt_src && spec.param.Wl_snk ) 
	{
	  ...
	  for(int m=0; m < files.had[i].baryon_PW.size(); ++m)
	  {
	    ...
	    files.had[i].baryon_PW[m].mom.resize(spec.had[i].baryon_PW[m].mom.size());
	
	    for(int n = 0; n < spec.had[i].baryon_PW[m].mom.size(); ++n)
	    {
	      files.had[i].baryon_PW[m].mom[n].filename = baryon_particle[m] 
		+ mom_name(spec.had[i].baryon_PW[m].mom[n].sink_mom) 
	...
	if ( spec.had[i].Sl_src && spec.param.Pt_snk ) 
	{
	  ...
	  for(int m=0; m < files.had[i].baryon_SP.size(); ++m)
	  {
	    ...
	    files.had[i].baryon_SP[m].mom.resize(spec.had[i].baryon_SP[m].mom.size());

	    for(int n = 0; n < spec.had[i].baryon_SP[m].mom.size(); ++n)
	    {
	      files.had[i].baryon_SP[m].mom[n].filename = baryon_particle[m] 
		+ mom_name(spec.had[i].baryon_SP[m].mom[n].sink_mom) 
		+ Mass_123 + ".D" + src_wvf_param_s[i2] + ms + ".P" + ms + ".SP";

//	    cout << spec.had[i].baryon_SP[m].mom[n].filename << endl;
	    }
	  }
	}
	...
	if ( spec.had[i].Sl_src && spec.param.Sl_snk ) 
	{
	  ...
	  for(int m=0; m < files.had[i].baryon_SS.size(); ++m)
	  {
	    ...
	    files.had[i].baryon_SS[m].mom.resize(spec.had[i].baryon_SS[m].mom.size());

	    for(int n = 0; n < spec.had[i].baryon_SS[m].mom.size(); ++n)
	    {
	      files.had[i].baryon_SS[m].mom[n].filename = baryon_particle[m] 
		+ mom_name(spec.had[i].baryon_SS[m].mom[n].sink_mom) 
...
//	    cout << spec.had[i].baryon_SP[m].mom[n].filename << endl;
	...
	if ( spec.had[i].Sl_src && spec.param.Wl_snk ) 
	{
	  ...
	  for(int m=0; m < files.had[i].baryon_SW.size(); ++m)
	  {
	    ...
	    files.had[i].baryon_SW[m].mom.resize(spec.had[i].baryon_SW[m].mom.size());

	    for(int n = 0; n < spec.had[i].baryon_SW[m].mom.size(); ++n)
	    {
	      files.had[i].baryon_SW[m].mom[n].filename = baryon_particle[m] 
		+ mom_name(spec.had[i].baryon_SW[m].mom[n].sink_mom) 
		+ Mass_123 + ".D" + src_wvf_param_s[i2] + ms + ".W" + ms + ".SW";

//	    cout << spec.had[i].baryon_SW[m].mom[n].filename << endl;
	    }
	  }
	}
	...
	if ( spec.had[i].Wl_src && spec.param.Pt_snk ) 
	{
	  ...
	  for(int m=0; m < files.had[i].baryon_WP.size(); ++m)
	  {
	    ...
	    files.had[i].baryon_WP[m].mom.resize(spec.had[i].baryon_WP[m].mom.size());
	
	    for(int n = 0; n < spec.had[i].baryon_WP[m].mom.size(); ++n)
	    {
	      files.had[i].baryon_WP[m].mom[n].filename = baryon_particle[m] 
		+ mom_name(spec.had[i].baryon_WP[m].mom[n].sink_mom) 
	...
	if ( spec.had[i].Wl_src && spec.param.Sl_snk ) 
	{
	  ...
	  for(int m=0; m < files.had[i].baryon_WS.size(); ++m)
	  {
	    ...
	    files.had[i].baryon_WS[m].mom.resize(spec.had[i].baryon_WS[m].mom.size());

	    for(int n = 0; n < spec.had[i].baryon_WS[m].mom.size(); ++n)
	    {
	      files.had[i].baryon_WS[m].mom[n].filename = baryon_particle[m] 
		+ mom_name(spec.had[i].baryon_WS[m].mom[n].sink_mom) 
	...
	if ( spec.had[i].Wl_src && spec.param.Wl_snk ) 
	{
	  ...
	  for(int m=0; m < files.had[i].baryon_WW.size(); ++m)
	  {
	    ...
	    files.had[i].baryon_WW[m].mom.resize(spec.had[i].baryon_WW[m].mom.size());
	
	    for(int n = 0; n < spec.had[i].baryon_WW[m].mom.size(); ++n)
	    {
	      files.had[i].baryon_WW[m].mom[n].filename = baryon_particle[m] 
		+ mom_name(spec.had[i].baryon_WW[m].mom[n].sink_mom) 
...
int main(int argc, char **argv)
{
  ...
  string xml_in_root = "/spectrumOct_w";
...
//  cout << "spec.had.meson_SP.mom.mesprop.size = " << spec.had[0].meson_SP[0].mom[0].mesprop.size() << endl;
  ...
  if (spec.param.wvf_param.size()/2 != spec.had.size())
  {
