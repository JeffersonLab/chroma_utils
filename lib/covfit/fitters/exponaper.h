//$Id: exponaper.h,v 2.0 2008/12/05 04:43:33 edwards Exp $
//$Log: exponaper.h,v $
...
//
//Revision 1.3  2005/05/13 20:20:13  kostas
//did nothing...
//
...
//
//Revision 1.1  2005/03/04 15:02:55  kostas
//*** empty log message ***
//
//
/*! Definition of the sum of exponentials with anti-periodic BC function 
  It's a sinh function....
*/


#ifndef EXPONENTIALS_APERIODIC
#define EXPONENTIALS_APERIODIC
//#define FUNCTION_NAME ExponentialsAPeriodic
#include "covfit/Function.h"

namespace CovFit {

class ExponentialsAPeriodic: public Function{
private:
  double center ; // the t=0 point
  double period ; // periodic extent of the lattice
public:
  
  ExponentialsAPeriodic():Function(){}
  ExponentialsAPeriodic(int n):center(0.0),period(0.0),Function(2*n){f_init();}
  ExponentialsAPeriodic(double c):center(c),period(0.0),Function(){f_init();}
  ExponentialsAPeriodic(double p, int n):center(0.0),period(p),Function(2*n){f_init();}
  ExponentialsAPeriodic(double c, double p):center(c),period(p),Function(){f_init();}
  ExponentialsAPeriodic(double c, double p,int n):center(c),period(p),Function(2*n){f_init();}
  
  virtual ~ExponentialsAPeriodic(){}
  ...
  void f_init(){
    if(npar%2!=0)
      std::cerr<<"ExponentialsAPeriodic::f_init : npar has to be even\n";
  }
  void f_init(FILE *fp)
  {
    int n ;
    fprintf(stderr,"sum of exponentials fitting with anti-periodic b.c.\n");
