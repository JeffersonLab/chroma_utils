//$Id: gAKfunc.h,v 2.0 2008/12/05 04:43:33 edwards Exp $
//$Log: gAKfunc.h,v $
...
#ifndef CHIPT_gAKfunc
#define CHIPT_gAKfunc
#include "covfit/Function.h"

namespace CovFit {

class gAKfunc: public Function{
private:
    double Delta ;
    double mu2 ;
    double Delta2 ;
public:
  ...
  gAKfunc(double D, double sc):Function(0),Delta(D/sc){
  ...
  void f_init(FILE *fp)
  {
    ...
    fprintf(stderr,"K(m,Delta,mu)  =") ;
    fprintf(stderr,"(m^2-2/3D^2)log(m^2/mu^2) + 2/3 Dsqrt(D^2-m^2)*log((D - sqrt(D^2-m^2)/(D + sqrt(D^2-m^2))))\n    + 2/3 m^2/D*(pi*m - sqrt(D^2-m^2)*log((D - sqrt(D^2-m^2)/(D + sqrt(D^2-m^2)))))\n     x is m^2/mu^2\n  Delta, m and mu should be in the same units");
  }
  
#define _PI_ 3.14159265358979323848
  ...
  double f(double x, double *b)
	...
	//std::cout<<"rootLog: "<<sqrtChiLog<<std::endl;
      ...
      return (x-2.0/3.0*Delta2)*chiLog + 2.0/3.0*Delta*sqrtChiLog  + 
	2.0/3.0*(_PI_*sqrt(x) - sqrtChiLog);
