// -*- C++ -*-
// $Id: ensem_primseed.h,v 2.0 2008/12/05 04:43:34 edwards Exp $
...
//! Binary input
template<class T>  
void read(ADATIO::BinaryReader& bin, PSeed<T>& d)
{
  for(int i=0; i < 4; ++i)
    read(bin, d.elem(i));
}
...
template<class T>
inline
void write(ADATIO::BinaryWriter& bin, const PSeed<T>& d)
{
  for(int i=0; i < 4; ++i)
    write(bin, d.elem(i));
}
...
template<class T>
inline
ENSEM::TextReader& operator>>(ENSEM::TextReader& txt, PSeed<T>& d)
{
  for(int i=0; i < 4; ++i)
    txt >> d.elem(i);
...
template<class T>
inline
ENSEM::TextWriter& operator<<(ENSEM::TextWriter& txt, const PSeed<T>& d)
{
  for(int i=0; i < 4; ++i)
    txt << d.elem(i) << "\n";
...
template<class T>
inline
ADATXML::XMLWriter& operator<<(ADATXML::XMLWriter& xml, const PSeed<T>& d)
{
  xml.openTag("Seed");
...
template<class T>
inline
void read(ADATXML::XMLReader& xml, const std::string& path, PSeed<T>& d)
{
...
template<class T>
struct EnsemScalar<PSeed<T> > {
  typedef PSeed<typename EnsemScalar<T>::Type_t>  Type_t;
};

// Traits class to label IO types
template<class T>
struct EnsbcIO<PSeed<T> > {
