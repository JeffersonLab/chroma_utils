// -*- C++ -*-
// $Id: ensem_primmatrix.h,v 2.0 2008/12/05 04:43:34 edwards Exp $
...
inline
void read(ADATIO::BinaryReader& bin, PMatrix<T,N,C>& d)
{
  for(int j=0; j < N; ++j)
    for(int i=0; i < N; ++i)
      read(bin, d.elem(i,j));
...
template<class T, int N, template<class,int> class C>  
inline
void write(ADATIO::BinaryWriter& bin, const PMatrix<T,N,C>& d)
{
  for(int j=0; j < N; ++j)
    for(int i=0; i < N; ++i)
      write(bin, d.elem(i,j));
}

//! Text input
template<class T, int N, template<class,int> class C>  
inline
ENSEM::TextReader& operator>>(ENSEM::TextReader& txt, PMatrix<T,N,C>& d)
{
  for(int j=0; j < N; ++j)
    for(int i=0; i < N; ++i)
      txt >> d.elem(i,j);
...
template<class T, int N, template<class,int> class C>  
inline
ENSEM::TextWriter& operator<<(ENSEM::TextWriter& txt, const PMatrix<T,N,C>& d)
{
  for(int j=0; j < N; ++j)
    for(int i=0; i < N; ++i)
      txt << d.elem(i,j);
...
template<class T, int N, template<class,int> class C>  
inline
ADATXML::XMLWriter& operator<<(ADATXML::XMLWriter& xml, const PMatrix<T,N,C>& d)
{
  xml.openTag("Matrix");
