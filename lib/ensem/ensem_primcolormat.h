// -*- C++ -*-
// $Id: ensem_primcolormat.h,v 2.1 2009/10/27 02:11:14 edwards Exp $
...
#ifndef ENSEM_PRIMCOLORMAT_H
#define ENSEM_PRIMCOLORMAT_H
...
// Makes a ensem scalar leaving primitive indices along
template<class T, int N>
struct EnsemScalar<PColorMatrix<T,N> > {
  typedef PColorMatrix<typename EnsemScalar<T>::Type_t, N>  Type_t;
};
...
colorContract(const PColorMatrix<T1,1>& s1, const PColorMatrix<T2,1>& s2, const PColorMatrix<T3,1>& s3)
{
  ...
  // not written 
  std::cerr << __func__ << ": not written for Nc=1" << std::endl;
  exit(1);
...
colorContract(const PColorMatrix<T1,2>& s1, const PColorMatrix<T2,2>& s2, const PColorMatrix<T3,2>& s3)
{
  ...
  // not written 
  std::cerr << __func__ << ": not written for Nc=2" << std::endl;
  exit(1);
...
colorContract(const PColorMatrix<T1,4>& s1, const PColorMatrix<T2,4>& s2, const PColorMatrix<T3,4>& s3)
{
  ...
  // not written 
  std::cerr << __func__ << ": not written for Nc=4" << std::endl;
  exit(1);
