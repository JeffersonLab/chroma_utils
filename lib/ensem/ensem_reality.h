// -*- C++ -*-
// $Id: ensem_reality.h,v 2.0 2008/12/05 04:43:34 edwards Exp $
...
//! Binary input
template<class T>  
void read(ADATIO::BinaryReader& bin, RScalar<T>& d)
{
  read(bin, d.elem());
}
...
template<class T>
inline
void write(ADATIO::BinaryWriter& bin, const RScalar<T>& d)
{
  write(bin, d.elem());
}
...
template<class T>
inline
ENSEM::TextReader& operator>>(ENSEM::TextReader& txt, RScalar<T>& d)
{
  return txt >> d.elem();
}
...
template<class T>
inline
ENSEM::TextWriter& operator<<(ENSEM::TextWriter& txt, const RScalar<T>& d)
{
  return txt << d.elem();
}
...
template<class T>
inline
ADATXML::XMLWriter& operator<<(ADATXML::XMLWriter& xml, const RScalar<T>& d)
{
  return xml << d.elem();
}
...
template<class T>
inline
void read(ADATXML::XMLReader& xml, const std::string& path, RScalar<T>& d)
{
...
private:
  T re;
  T im;
} ENSEM_ALIGN8;   // possibly force alignment
...
template<class T>  
inline
void read(ADATIO::BinaryReader& bin, RComplex<T>& d)
{
  read(bin, d.real());
  read(bin, d.imag());
}
...
template<class T>
inline
void write(ADATIO::BinaryWriter& bin, const RComplex<T>& d)
{
  write(bin, d.real());
  write(bin, d.imag());
}
...
template<class T>
inline
std::istream& operator>>(std::istream& s, RComplex<T>& d)
{
  while(s.get() != '(') {}
  s >> d.real();
  while(s.get() != ',') {}
...
template<class T>
inline
ENSEM::TextReader& operator>>(ENSEM::TextReader& s, RComplex<T>& d)
{
...
template<class T> 
inline  
ENSEM::TextWriter& operator<<(ENSEM::TextWriter& s, const RComplex<T>& d)
{
  return s << d.real() << " " << d.imag();
}
...
template<class T>
inline
ADATXML::XMLWriter& operator<<(ADATXML::XMLWriter& xml, const RComplex<T>& d)
{
  xml.openTag("re");
  xml << d.real();
  xml.closeTag();
...
template<class T>
inline
void read(ADATXML::XMLReader& xml, const std::string& xpath, RComplex<T>& d)
{
...
template<class T>
struct EnsemScalar<RScalar<T> > {
  typedef RScalar<typename EnsemScalar<T>::Type_t>  Type_t;
};

template<class T>
struct EnsemScalar<RComplex<T> > {
  typedef RComplex<typename EnsemScalar<T>::Type_t>  Type_t;
};
...
template<class T> 
struct EnsbcIO<RScalar<T> > {
  enum {type = 0};
};

template<class T> 
struct EnsbcIO<RComplex<T> > {
  enum {type = 1};
};
...
// RScalar
template<class T1>
struct UnaryReturn<RScalar<T1>, FnMean> {
  typedef RScalar<typename UnaryReturn<T1, FnMean>::Type_t>  Type_t;
};

template<class T1>
struct UnaryReturn<RScalar<T1>, FnVariance> {
  typedef RScalar<typename UnaryReturn<T1, FnVariance>::Type_t>  Type_t;
};
...
// RComplex
template<class T1>
struct UnaryReturn<RComplex<T1>, FnMean> {
  typedef RComplex<typename UnaryReturn<T1, FnMean>::Type_t>  Type_t;
};

template<class T1>
struct UnaryReturn<RComplex<T1>, FnVariance> {
  typedef RScalar<typename UnaryReturn<T1, FnVariance>::Type_t>  Type_t;
};
...
template<class T>
bool
isInf(const RScalar<T>& s1)
{
  return isInf(s1.elem());
}


//! bool = isFinite(RScalar)
template<class T>
bool
isFinite(const RScalar<T>& s1)
{
...
template<class T>
bool
isZero(const RComplex<T>& s1)
{
  return isZero(s1.real()) && isZero(s1.imag());
}
...
template<class T>
bool
isInf(const RComplex<T>& s1)
{
  return isInf(s1.real()) || isInf(s1.imag());
}


//! bool = isFinite(RComplex)
template<class T>
bool
isFinite(const RComplex<T>& s1)
{
