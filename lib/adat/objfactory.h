// -*- C++ -*-
// $Id: objfactory.h,v 2.2 2009/06/13 00:05:34 edwards Exp $
...
#include "adat/typeinfo.h"
#include "adat/typelist.h"
#include <iostream>
#include <sstream>
#include <map>
#include <vector>
 ...
 */
  ...
  public:
    ...
    // Member functions 
    //! Does this key exist?
    bool exist(const IdentifierType& id) const {
      return (associations_.find(id) == associations_.end()) ? false : true;
    }

    //! Dump keys
    std::vector<IdentifierType> keys() const {
      std::vector<IdentifierType> _keys;
      typedef typename IdToProductMap::const_iterator CI;
      for( CI j = associations_.begin();
	   j != associations_.end(); j++) {
	_keys.push_back(j->first);
      }
	...
	typedef typename IdToProductMap::const_iterator CI;
	std::cerr << "Couldnt find key " << id << " in the map: " << std::endl;
	std::cerr << "Available Keys are : " << std::endl;
	for( CI j = associations_.begin();
	     j != associations_.end(); j++) {
	  std::cerr << j->first << std::endl << std::flush;
	}
