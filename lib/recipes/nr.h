...
namespace NR 
{
  //JJD added virtual function to make zbrent, zbrak, zbrac easier to thread 
  //expanded to include mnbrak and brent 
  class ScalFunc
  {
  public:
    virtual DP operator()(const DP x) const = 0;
    virtual ~ScalFunc(){}
  };

  // this one supposed to take a pointer to a function
  class PtrScalFunc : public ScalFunc
  {
  public:
    PtrScalFunc(DP (*f)(DP) ) : F(f){};
 
    DP operator()(const DP x) const { return (*F)(x); }
  ...
  //JJD added virtual function for mnewt
  class ScalFuncMultiVars
  {
  public:
    virtual Vec_O_DP values(const Vec_DP x) const = 0;
    virtual Mat_O_DP jacobian(const Vec_DP x) const = 0;
    virtual ~ScalFuncMultiVars(){}
  };

  


  bool zbrac(const ScalFunc &func, DP &x1, DP &x2);
  bool zbrac(DP func(const DP), DP &x1, DP &x2);

  void zbrak(const ScalFunc &fx, const DP x1, const DP x2, const int n,
	     Vec_O_DP &xb1, Vec_O_DP &xb2, int &nroot);
  void zbrak(DP fx(const DP), const DP x1, const DP x2, const int n,
	     Vec_O_DP &xb1, Vec_O_DP &xb2, int &nroot);

  //added by DJW
  bool zbrac_pos(const ScalFunc &func, DP &x1, DP &x2);
  bool zbrac_pos(DP func(const DP), DP &x1, DP &x2);

  //added by jjd
  bool zbrac_neg(const ScalFunc &func, DP &x1, DP &x2);
  bool zbrac_neg(DP func(const DP), DP &x1, DP &x2);

  DP zbrent(const ScalFunc &func, const DP x1, const DP x2, const DP tol);
  DP zbrent(DP func(const DP), const DP x1, const DP x2, const DP tol);

  DP brent(const DP ax, const DP bx, const DP cx, const ScalFunc &f,
  ...
  void arcmak(Vec_I_ULNG &nfreq, unsigned long nchh, unsigned long nradd,
	      arithcode &acode);
  void arcode(unsigned long &ich, std::string &code, unsigned long &lcd,
  ...
  void chstwo(Vec_I_DP &bins1, Vec_I_DP &bins2, const int knstrn, DP &df,
	      DP &chsq, DP &prob);
  void cisi(const DP x, std::complex<DP> &cs);
  ...
  void frenel(const DP x, std::complex<DP> &cs);
  ...
  void hufdec(unsigned long &ich, std::string &code, const unsigned long lcode,
	      unsigned long &nb, huffcode &hcode);
  void hufenc(const unsigned long ich, std::string &code, unsigned long &nb,
	      huffcode &hcode);
  ...
  std::complex<DP> hypgeo(const std::complex<DP> &a, const std::complex<DP> &b,
			  const std::complex<DP> &c, const std::complex<DP> &z);
  void hypser(const std::complex<DP> &a, const std::complex<DP> &b,
	      const std::complex<DP> &c, const std::complex<DP> &z,
	      std::complex<DP> &series, std::complex<DP> &deriv);
  unsigned short icrc(const unsigned short crc, const std::string &bufptr,
  ...
  void laguer(Vec_I_CPLX_DP &a, std::complex<DP> &x, int &its);
  ...
  void moment(Vec_I_DP &data, DP &ave, DP &adev, DP &sdev, DP &var, DP &skew,
	      DP &curt);
  void mp2dfr(Vec_IO_UCHR &a, std::string &s);
  ...
  void spctrm(std::ifstream &fp, Vec_O_DP &p, const int k, const bool ovrlap);
